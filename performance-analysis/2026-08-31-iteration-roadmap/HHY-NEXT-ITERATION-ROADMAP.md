...
用真实、复杂、跨平台的官方扩展验证 v1.2 工具链和 Process Extension Protocol，并在同一版本内只修复真实集成证明的协议瓶颈，而不是用示例扩展宣布生态成熟或根据想象开放全部预留能力。

#### 扩展选择

确定使用官方 HTML 扩展，因为它能同时验证：

- 不可信、畸形和大体积 HTML 的安全解析；
- DOM、CSS 选择器和嵌套结构化数据投影；
- 单次解析、多字段批量抽取和有界结果生命周期；
- 无文件、网络和子进程权限的最小 capability；
- 结构化 Error、输入上限和确定性失败；
- Lexbor 等第三方依赖的打包和跨平台发行；
- 大结果集对 Stream credit、取消或 Opaque Handle 的真实需求验证。

HTML 扩展必须使用真实网页 fixture 和批量抽取负载完成验收，不能退化为只回显同步 JSON 的示例扩展。

#### 功能边界

- 只解析调用方显式传入的 HTML，不自行发起 HTTP、读取文件或执行页面脚本；
- CSS 选择器、schema 字段、输入字节数和返回条目数均有硬上限；
- 优先单次解析后的批量字段投影，避免逐节点跨进程调用；
- 扩展保持 `effect = none`，manifest 的文件、网络和 process capability 全部为空；
...
| Opaque Handle | 解析后的 DOM 或选择器游标需跨调用复用且不能安全序列化 |
...
- 记录协议缺口、可接受规避方式以及每项候选能力的采用或不采用结论；
- 每项新增能力关联至少一个真实集成证据；
- 旧扩展无需重发即可运行；
- 新旧 Core/扩展组合有兼容矩阵；
- 取消、backpressure 和 handle 泄漏有压力测试；
- 仍优先使用进程隔离，不因单个扩展的便利性直接公开 Runtime C ABI。

#### 实施与验收记录

- [x] `html.extract_report` 在一次 DOM 解析中完成多字段投影，并返回 `matched`、`returned`、`truncated` 与 `input_bytes`；
- [x] 真实目录页 fixture、畸形 HTML、非法 CSS selector、输入/结果硬上限和稳定错误进入 Release/Debug suite；
- [x] Protocol 1 以可忽略可选字段兼容增加 `operation`、`stage`、`cause`，旧扩展无需重发；
- [x] manifest 保持 filesystem、network、process capability 全空，扩展不抓取 URL、不读取文件、不执行脚本；
...
- 至少两个相互独立的真实集成；
- 已使用 v1.2.2 按真实证据提供的批量、流式、取消或 handle 能力；
