...
| v1.2.0 | **实施中 · 2026-08-31** | 官方扩展分发与签名 | 包身份、Ed25519 签名、依赖解析、静态官方索引、事务式安装 | 来源和依赖可验证；篡改包拒绝；失败安装不破坏现有环境 |
...
> **状态：实施中（2026-08-31）**<br>
> 已实现：静态签名 Registry、显式 Ed25519 信任根、索引与包描述双层签名、确定性传递依赖解析、全图校验、`--dry-run`、同文件系统 staging、原子单包提交和失败图回滚。<br>
> 待完成门槛：完整跨平台 CI、正式提交证据和官网一致性验证。暂不创建 Git tag、GitHub Release 或正式发布包。
...
- 干净机器可凭 lockfile 和离线 bundle 重建环境；
- 中断、磁盘不足、验签失败和启动失败不破坏旧版本；
- rollback 通过故障注入测试；
- cache、lock、active installation 可由 `doctor` 核对；
- 官网承诺的签名、依赖、索引、离线锁定、可复现安装和回滚全部闭环。
