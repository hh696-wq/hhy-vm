...
const siteGraphCode = `try {
    ...
    let report = build_report(
        config.project, crawl_result,
        inventory, graph, failures(crawl_result)
    )
    # ... 原子写入 inventory、graph 与 failures ...
    report |> encode_json({ pretty: true })
        |> save_text(path(args[3]), { atomic: true })
    if not report.ok { exit(1) }
} catch err { print_error(err); exit(1) }`;
    ...
    <main className="editorial-home">
      <section className="editorial-hero section-shell">
        ...
        <article className="featured-project">
          ...
          <pre className="project-output hhy-source" aria-label={zh ? "SiteGraph Auditor HHY 源码示例" : "SiteGraph Auditor HHY source example"}><code><HhyCode code={siteGraphCode} lineNumbers /></code></pre>
