              ...
              if (block.type === "evolution-roadmap") {
                const released: Array<{ version: string; date: string; title: string; detail: string; icon: ElementType }> = lang === "zh"
                  ? [
                    ...
                    { version: hhyVersionTag, date: "2026-09-02", title: "Runtime 错误路径加固", detail: "结构化错误完整构造后发布；原始 fuzz 输入纳入永久回归，四平台门禁通过", icon: ShieldCheck }
                  ]
                  : [
                    ...
                    { version: hhyVersionTag, date: "2026-09-02", title: "Runtime error-path hardening", detail: "Structured errors publish after full construction; the original fuzz input is retained and four-platform gates pass", icon: ShieldCheck }
                  ];
                ...
                return (
                  <figure className="evolution-roadmap" key={index}>
                    ...
                    <figcaption>{lang === "zh" ? `演进顺序：v1.3.5 Bytecode 默认 → v1.3.6 fusion → v1.3.7 特化加固 → v1.3.8 优化 IR → v1.3.9 观测一致性 → v1.3.10 缓存治理 → ${hhyVersionTag} Runtime 加固 → 生态 ABI 决策` : `Evolution order: v1.3.5 Bytecode default → v1.3.6 fusion → v1.3.7 specialization hardening → v1.3.8 optimization IR → v1.3.9 observability consistency → v1.3.10 cache governance → ${hhyVersionTag} Runtime hardening → ecosystem ABI decision`}</figcaption>
                  </figure>
                );
              }
