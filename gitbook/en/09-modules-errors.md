...
Only exported names are visible to importers. A module owns its top-level scope, executes once on first import, and is then cached. Import cycles raise CheckError before execution. V1.7.0 supports standard and local modules plus process-extension modules installed from local packages.
