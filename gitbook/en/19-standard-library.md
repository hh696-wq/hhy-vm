# 19. Standard Library Function Index

Signatures and purposes for all 121 V1.7.0 core callables in the runtime Registry.

## 19.1 Reading the signatures

This page is sourced from the V1.7.0 Runtime Callable Contract Registry and contains all 121 core callables; dynamically registered callables are documented by their extensions. T/U are generic placeholders, ? marks an optional argument or nullable result, and Map? is an optional options Map. Every function supports ordinary calls; a pipe injects its left value as the first argument.
...
## 19.2 Core values, collections, environment, and control (22)
...
## 19.4 Text, Regex, JSON, and CSV (18)
...
## 19.6 Processes, standard input, and timers (6)
...
web.* builds persistent applications, responses, middleware, and production endpoints; only web.listen binds a network port.
...
```text
web.get(WebApp, String, Function) -> WebApp
```
...
```text
web.post(WebApp, String, Function) -> WebApp
```
...
```text
web.put(WebApp, String, Function) -> WebApp
```
...
```text
web.patch(WebApp, String, Function) -> WebApp
```
...
```text
web.delete(WebApp, String, Function) -> WebApp
```
...
```text
web.use(WebApp, Function) -> WebApp
```

Append short-circuiting middleware.

### `web.static`

```text
web.static(WebApp, String, Path) -> WebApp
```

Mount a secure static directory.

### `web.cors`

```text
web.cors(WebApp, Map?) -> WebApp
```

Enable CORS and OPTIONS preflight.

### `web.request_id`

```text
web.request_id(WebApp) -> WebApp
```
...
```text
web.health(WebApp, String?) -> WebApp
```
...
```text
web.trust_proxy(WebApp) -> WebApp
```

Trust the proxy-provided client address.

### `web.text`

```text
web.text(String, Map?) -> WebResponse
```
...
```text
web.html(String, Map?) -> WebResponse
```
...
```text
web.json(JsonValue, Map?) -> WebResponse
```
...
```text
web.redirect(String, Map?) -> WebResponse
```
...
```text
web.response(Map) -> WebResponse
```

Create a response from an options Map.

### `web.multipart`

```text
web.multipart(WebRequest, Path?) -> Map
```

Parse bounded multipart forms and uploads.

### `web.stream`

```text
web.stream(Stream, Map?) -> WebResponse
```

Create a backpressured streaming response.

### `web.sse`

```text
web.sse(Stream<String>) -> WebResponse
```
...
```text
web.sign_cookie(String, String) -> String
```

HMAC-sign a cookie value.

### `web.verify_cookie`

```text
web.verify_cookie(String, String) -> String | Null
```

Verify and read a signed cookie.

### `web.metrics`

```text
web.metrics(WebApp, String?) -> WebApp
```
...
```text
web.gzip(WebApp) -> WebApp
```

Enable gzip negotiation through Accept-Encoding.

### `web.listen`

```text
web.listen(WebApp, Map?) -> Null
```

Start the bounded HTTP/1.1 server.
