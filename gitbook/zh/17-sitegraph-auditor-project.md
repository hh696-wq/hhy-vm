# 17. 实战项目：SiteGraph Auditor

递归建立文档站页面清单与规范化链接图，用 metadata、失败和安全边界实施质量门禁。

## 17.1 一个真正使用安全 Spider 的质量门禁

SiteGraph Auditor 基于 HHY v1.7.0 和 my-crawler 的安全递归引擎。它从 seed 开始逐层发现页面，同时输出 inventory、规范链接图、report 和 failures；报告以稳定退出码阻止缺失 metadata 或存在抓取失败的站点通过。
