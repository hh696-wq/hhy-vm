...
用 HHY v1.7.0 构建完整的代码仓库体检与质量门禁应用，包含真实数据、并发检查、JSON 报告和端到端测试。

## 12.1 这不是语法 Demo

FlowGuard 是一个使用 HHY v1.7.0 运行并通过自测的完整应用。它接收项目目录与 JSON 配置，检查必需文件、扫描文件和疑似凭据、并发执行质量命令与 HTTP 健康检查，最终原子写入结构化报告，并用稳定退出码决定质量门禁是否通过。
