        ...
        if (parsed.ok && (command == COMMAND_BYTECODE ||
            ...
            if (parsed.ok) {
                ...
                if (prepared.ok && command == COMMAND_BYTECODE)
                    ...
                    json_object_set_new(metrics, "typed_plan_version", json_integer(HHY_TYPED_VERSION));
                    json_object_set_new(metrics, "typed_plans", json_integer((json_int_t)chunk.typed_plan_count));
                    json_object_set_new(metrics, "typed_plan_bytes", json_integer(
                        (json_int_t)(chunk.typed_plan_count * sizeof(*chunk.typed_plans))));
