...
- `bytecode` 编译、验证并反汇编内部 Bytecode IR，但不执行；`bytecode --metrics`
  输出 schema-1 的 compile/verify/prepare 缓存准入测量。opcode、内存布局和磁盘格式
  不构成公开兼容承诺。v1.3.5 起 `run` 默认使用 Bytecode，`--engine ast` 和
  `HHY_ENGINE=ast` 永久保留 AST 语义 oracle 与紧急回退。v1.3.10 的真实数据未触发
  缓存准入，因此不接受外部预编译 Bytecode，也没有 `.hhyc` 缓存。
