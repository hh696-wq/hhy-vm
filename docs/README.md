...
| 评估项目现状与后续工作 | [当前版本说明](CURRENT_VERSION.md) → [主迭代路线图](ROADMAP.md) |
...
- 演示：[终端演示脚本](../scripts/terminal-demo.sh)。推广草稿、设计素材和视频留在主仓之外。

## 版本记录与规划

当前发行说明见 [1.7.0](https://github.com/hh696-wq/hhy-vm/releases/tag/v1.7.0)，前序关键版本见 [1.5.0](https://github.com/hh696-wq/hhy-vm/releases/tag/v1.5.0) 和 [1.4.3](https://github.com/hh696-wq/hhy-vm/releases/tag/v1.4.3)。[历史发行索引](https://github.com/hh696-wq/hhy-vm/releases) 汇总旧版本，保留原始日期、结论及证据。

当前迭代顺序与完成标准维护在[公共路线图](ROADMAP.md)。当前下一重点是 v1.8 外部采用与旗舰场景，优化默认准入继续单独按数据评估。
...
6. 本目录仅跟踪核心使用、规范与开发文档。`releases/`、`EXTENSION_ROADMAP.md`、`LANGUAGE_HEALTH_REPORT.md`、`TERMINAL_DEMO.md` 及推广草稿只保留本地，由 `.gitignore` 排除。原始性能分析、官网、完整案例和编辑器源码留在本地或独立维护。修改相对链接后运行 `python3 scripts/check-repository.py`，该检查只认可 Git 跟踪的目标。
