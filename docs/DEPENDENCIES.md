...
| zlib | Web 响应 gzip 压缩 | zlib License | `zlib.h` 与 `libz` | [zlib](https://zlib.net/) |
...
`brew install curl pcre2 bdw-gc jansson openssl@3 zlib`；database 扩展另需 `libpq mysql`，
html 扩展另需 `lexbor`。
Linux 使用发行版对应的开发包。
