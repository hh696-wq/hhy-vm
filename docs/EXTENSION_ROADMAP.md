...
> 状态：v1.7.0 当前实施摘要 + 早期方案归档
> 当前 Core 版本：`1.7.0`（以仓库根目录 `VERSION` 为准）
...
| --- | --- | --- |
| v1.1 | 进程扩展与本地安装 | 已发布 |
| v1.2 | 签名 Registry、锁定、回滚、HTML 验证 | 已发布；并非下方早期 Office 设想 |
| v1.3 | Bytecode 默认、Verifier、Profiler 与资源治理 | 已发布 |
| v1.4.3 | 完整 Web Runtime | 已发布 |
| v1.5.0 | DB 1.0.0 池、远程 TLS、事务、游标、精确类型、作用域与取消 | 本地/CI/发行通过；RDS 和部署长稳待验证 |
| v1.7.0 | 优化编译器及 v1.6 Runtime 实验统一交付；Protocol 1 与 DB 1.0.0 独立版本保持兼容 | 已发布；优化默认关闭，第三方扩展未逐个验证 |

下一阶段优先验证官方模板的安装、lock 重建、离线与故障恢复；不新增 Office 或 Native ABI 的发布承诺。完整依赖与验收任务见[主路线图](ROADMAP.md)。

当前契约见 [Protocol 1](EXTENSION_PROTOCOL_V1.md) 与
[DB 1.0.0](../extensions/database/README.md)。**以下各节保留早期设计历史**，
其中 Office 的 v1.2 排期、94 callable 数量和“未来取消/句柄”等描述不代表当前实现或新的发布承诺。
