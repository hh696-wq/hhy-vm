#!/usr/bin/env python3
"""Paired local evidence for the experimental verified call-layout path."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    parser.add_argument('--output', default='build/benchmarks/call-layout.json')
    ...
    results = []
    with tempfile.TemporaryDirectory(prefix='hhy-call-layout-bench-') as temp:
        ...
        for source in sources:
            ...
            for iteration in range(args.iterations + 2):
                ...
                for mode in order:
                    env = {**os.environ, 'HHY_BYTECODE_CALL_PLANS': '1' if mode == 'on' else '0', 'HHY_PROFILE_DISPATCH': '0'}
            ...
            for mode, binary in binaries.items():
                run = subprocess.run([binary, 'profile', '--engine', 'bytecode', '--heap', '--format', 'json', '--output', str(profile_file), source],
                                     env={**os.environ, 'HHY_BYTECODE_CALL_PLANS': '1' if mode == 'on' else '0', 'HHY_PROFILE_DISPATCH': '0'}, capture_output=True, check=True)
                if (run.stdout, run.stderr) != (oracle.stdout, oracle.stderr):
    ...
    report = {'schema_version': 1, 'platform': platform.platform(), 'machine': platform.machine(),
              ...
              'decision': {'default_enabled': False, 'status': 'experimental_only',
                           ...
                           'tail_calls': 'not_eliminated', 'unwind_table': 'not_implemented'}}
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(report, indent=2) + '\n')
    print(f'Call-layout evidence saved to {output}; default remains off')
