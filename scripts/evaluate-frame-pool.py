#!/usr/bin/env python3
"""Paired local evidence for the experimental bounded call-frame pool."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    parser.add_argument('--output', default='build/benchmarks/frame-pool.json')
    ...
    results = []
    with tempfile.TemporaryDirectory(prefix='hhy-frame-pool-bench-') as temp:
        profile_file = Path(temp) / 'profile.json'
        retention = Path(temp) / 'retention.hhy'
        retention.write_text('fn wide(n) {\n' + '\n'.join(f'let v{i} = n + {i}' for i in range(100)) +
                             '\nif n == 0 { return v99 }\nreturn wide(n - 1)\n}\nprint(wide(128))\n')
        sources.append(str(retention))
        for source in sources:
            ...
            for iteration in range(args.iterations + 2):
                ...
                for mode in order:
                    env = {**os.environ, 'HHY_CALL_FRAME_POOL': 'bounded' if mode == 'on' else 'legacy', 'HHY_BYTECODE_CALL_PLANS': '0', 'HHY_BYTECODE_EXCEPTION_TABLES': '0', 'HHY_PROFILE_DISPATCH': '0'}
            ...
            for mode, binary in binaries.items():
                run = subprocess.run([binary, 'profile', '--engine', 'bytecode', '--heap', '--format', 'json', '--output', str(profile_file), source],
                                     env={**os.environ, 'HHY_CALL_FRAME_POOL': 'bounded' if mode == 'on' else 'legacy', 'HHY_BYTECODE_CALL_PLANS': '0', 'HHY_BYTECODE_EXCEPTION_TABLES': '0', 'HHY_PROFILE_DISPATCH': '0'}, capture_output=True, check=True)
                if (run.stdout, run.stderr) != (oracle.stdout, oracle.stderr):
            ...
            results.append({'source': source, 'source_sha256': hashlib.sha256(Path(source).read_bytes()).hexdigest(),
                            'source_text': Path(source).read_text(), 'synthetic': '/call-' in source or source == str(retention), 'wall_samples_ms': samples, 'wall_medians_ms': medians,
    ...
    report = {'schema_version': 1, 'platform': platform.platform(), 'machine': platform.machine(),
              ...
              'decision': {'default_enabled': False, 'status': 'experimental_only',
                           ...
                           'pool_limits': {'frames': 64, 'retained_gc_bytes': 65536}, 'native_frame_unwind': 'unchanged'}}
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(report, indent=2) + '\n')
    print(f'Frame-pool evidence saved to {output}; default remains off')
